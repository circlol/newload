...
Function Backup-ProgramData {
    ...
    $DetectedPrograms = "Detected Programs: $programsToBackup"

    Write-Output $TransferText
    Write-Output $DetectedPrograms
...
function Invoke-RoboCopy {
    begin {
        ...
        If ( $Textbox_Source.text -like $null ) {
            ...
            $description = "ATTENTION!!`nSelect the SOURCE folder.`n"
            $Global:SourceFolder = Get-Folder -Description $description
        ...
        If ( $Textbox_Destination.text -like $null ) {
            ...
            $description = "ATTENTION!!`nSelect the DESTINATION folder.`n"
            $Global:DestinationFolder = Get-Folder -Description $description
...
Function Start-ScanForPST {
    ...
    $Path3 = "$Env:LOCALAPPDATA"
    $Path4 = "$Env:APPDATA"

    # Scan user's documents folder for .pst files
    $UserProfilePSTs = Get-ChildItem -Path $Path1 -Include "*.pst" , "*.ost" -Recurse
    # Scan user's onedrive\documents folder for .pst files
    $OneDrivePSTs = Get-ChildItem -Path $Path2 -Include "*.pst" , "*.ost" -Recurse
    # Scan local AppData for .pst files
    $LocalAppDataPSTs = Get-ChildItem -Path $Path3 -Include "*.pst" , "*.ost" -Recurse
    # Scan roaming AppData for .pst files (note: you are using "*.ps1" as the filter here)
    $RoamingAppDataPSTs = Get-ChildItem -Path $Path4 -Include "*.pst" , "*.ost" -Recurse

    # Combines variables into an array
    $Global:PSTFiles = @()
    $PSTFiles += $UserProfilePSTs
    $PSTFiles += $OneDrivePSTs
    $PSTFiles += $LocalAppDataPSTs
    $PSTFiles += $RoamingAppDataPSTs
...
#        $PSTFiles
        $Title = "Outlook Data Files have been located"
        Write-Output "$Title. OST and PST files are listed below."
        foreach ($PSTFile in $PSTFiles) {
            Write-Host $PSTFile
        }
        Write-Output "`n`nThere will be a pop-up window asking if you want to backup."
        $Query = Show-Question -Title $Title -Message "$Title.  Would you like to back them up?" -YesNo -Icon Question
        If ( $Query -eq "Yes" ) {
