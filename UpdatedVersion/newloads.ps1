		...
		If (!$Skip) {
			If ($Variables.osVersion -like "*Windows 10*") {
				...
				Foreach ($StartBinFile in $StartBinFiles) {
					...
					xcopy $StartBinFile.FullName $Variables.StartBinDefault /y | Out-File $Variables.Log -Append
					Get-Status
					$progress++
					Write-Status "Copying $($StartBinFile.Name) to current user ($progress)/$TotalBinFiles)" "+" -NoNewLine
					xcopy $StartBinFile.FullName $Variables.StartBinCurrent /y | Out-File $Variables.Log -Append
					Get-Status
				}
			}
		}
