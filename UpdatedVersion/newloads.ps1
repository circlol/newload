...
Function Get-ADWCleaner {
    If ($SkipADW -or $Revert) {
        ...
        If (!(Test-Path $adwDestination)) {
            Write-Status -Types "+", "ADWCleaner" -Status "Downloading ADWCleaner"
            # - Downloads ADW
            Start-BitsTransfer -Source "$adwLink" -Destination "$adwDestination" -Dynamic
        }
...
Function Get-Office {
    ...
    If ($office32 -or $Office64 -eq $true) { $officecheck = $true }
    If ($officecheck -eq $true) { Write-Status -Types "WAITING" -Status "Office Exists" -WriteWarning }
    Else { 
...
Function Import-Variable {
    ...
    New-Variable -Name "Win11" -Value "22000" -Scope Global
    New-Variable -Name "22H2" -Value "22621" -Scope Global
    New-Variable -Name "OSVersion" -Value (Get-CimInstance -ClassName Win32_OperatingSystem).Caption -Scope Global
    New-Variable -Name "BuildNumber" -Value [System.Environment]::OSVersion.Version.Build -Scope Global
    New-Variable -Name "NetStatus" -Value (Get-NetConnectionProfile).IPv4Connectivity -Scope Global
    New-Variable -Name "Connected" -Value "Internet" -Scope Global

    # Local File Paths
    New-Variable -Name "HVECCodec" -Value  "$NewLoads\Microsoft.HEVCVideoExtension_2.0.60091.0_x64__8wekyb3d8bbwe.Appx" -Scope Global
    New-Variable -Name "WallpaperDestination" -Value "C:\Windows\Resources\Themes\mother.jpg" -Scope Global
    New-Variable -Name "WallpaperPath" -Value "$NewLoads\mother.jpg" -Option ReadOnly -Scope Global
    New-Variable -Name "ErrorLog" -Value "$env:userprofile\Desktop\New Loads Errors.Log" -Option ReadOnly -Scope Global
    New-Variable -Name "Log" -Value "$env:userprofile\Desktop\New Loads.Log" -Scope Global
    New-Variable -Name "adwDestination" -Value "$NewLoads\adwcleaner.exe" -Scope Global
    New-Variable -Name "DriverSelectorPath" -Value "$NewLoads\Driver Grabber.exe" -Scope Global
    New-Variable -Name "WindowsUpdatesPath" -Value "$NewLoads\Windows Updates.exe" -Scope Global
    New-Variable -Name "SaRA" -Value "$newloads\SaRA.zip" -Option ReadOnly -Scope Global
    New-Variable -Name "Sexp" -Value "$newloads\SaRA" -Option ReadOnly -Scope Global

    New-Variable -Name "PackagesRemoved" -Value @() -Scope Global
    New-Variable -Name "Removed" -Value 0 -Scope Global
    New-Variable -Name "Failed" -Value 0 -Scope Global
    New-Variable -Name "NotFound" -Value 0 -Scope Global

    New-Variable -Name "StartBinDefault" -Value "$Env:SystemDrive\Users\Default\AppData\Local\Packages\Microsoft.Windows.StartMenuExperienceHost_cw5n1h2txyewy\LocalState\" -Option ReadOnly -Scope Global
    New-Variable -Name "StartBinCurrent" -Value "$Env:userprofile\AppData\Local\Packages\Microsoft.Windows.StartMenuExperienceHost_cw5n1h2txyewy\LocalState" -Option ReadOnly -Scope Global
    New-Variable -Name "LayoutFile" -Value "$Env:LOCALAPPDATA\Microsoft\Windows\Shell\LayoutModification.xml" -Scope Global -Option ReadOnly


    New-Variable -Name "adwLink" -Value "https://github.com/circlol/newload/raw/main/adwcleaner.exe" -Scope Global
    New-Variable -Name "livesafe" -Value "$Env:PROGRAMFILES\McAfee\MSC\mcuihost.exe" -Option ReadOnly -Scope Global
    New-Variable -Name "WebAdvisor" -Value "$Env:PROGRAMFILES\McAfee\WebAdvisor\Uninstaller.exe" -Option ReadOnly -Scope Global
    New-Variable -Name "WildGames" -Value "${Env:PROGRAMFILES(x86)}\WildGames\Uninstall.exe" -Option ReadOnly -Scope Global
    New-Variable -Name "EdgeShortcut" -Value "$Env:USERPROFILE\Desktop\Microsoft Edge.lnk" -Option ReadOnly -Scope Global
    #New-Variable -Name "AcroSC" -Value "$Env:PUBLIC\Desktop\Adobe Acrobat DC.lnk" -Option ReadOnly -Scope Global
    New-Variable -Name "AcroSC" -Value "$Env:PUBLIC\Desktop\Acrobat Reader DC.lnk" -Option ReadOnly -Scope Global
    New-Variable -Name "EdgeSCPub" -Value "$Env:PUBLIC\Desktop\Microsoft Edge.lnk" -Option ReadOnly -Scope Global
    New-Variable -Name "VLCSC" -Value "$Env:PUBLIC\Desktop\VLC Media Player.lnk" -Option ReadOnly -Scope Global
    New-Variable -Name "ZoomSC" -Value "$Env:PUBLIC\Desktop\Zoom.lnk" -Option ReadOnly -Scope Global
    New-Variable -Name "CommonApps" -Value "$env:ALLUSERSPROFILE\Microsoft\Windows\Start Menu\Programs" -Option ReadOnly -Scope Global

    #Wallpaper

    #New-Variable -Name "Wallpaper" -Value "$env:appdata\Microsoft\Windows\Themes\MotherComputersWallpaper.jpg" -Option ReadOnly -Scope Global
    New-Variable -Name "CurrentWallpaper" -Value (Get-ItemProperty -Path "HKCU:\Control Panel\Desktop" -Name Wallpaper).Wallpaper -Option ReadOnly -Scope Global
    #Office Removal
    New-Variable -Name "PathToOffice86" -Value "${env:ProgramFiles(x86)}\Microsoft Office" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToOffice64" -Value "$env:ProgramFiles\Microsoft Office 15" -Option ReadOnly -Scope Global
    New-Variable -Name "OfficeCheck" -Value "$false" -Option ReadOnly -Scope Global
    New-Variable -Name "Office32" -Value "$false" -Option ReadOnly -Scope Global
    New-Variable -Name "Office64" -Value "$false" -Option ReadOnly -Scope Global

    #Reg

    New-Variable -Name "UsersFolder" -Value "{59031a47-3f72-44a7-89c5-5595fe6b30ee}" -Scope Global
    New-Variable -Name "ThisPC" -Value "{20D04FE0-3AEA-1069-A2D8-08002B30309D}" -Scope Global
    New-Variable -Name "PathToUblockChrome" -Value "HKLM:\Software\Wow6432Node\Google\Chrome\Extensions\cjpalhdlnbpafiamejdnhcphjbkeiagm" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToChromeLink" -Value "https://clients2.google.com/service/update2/crx" -Option ReadOnly -Scope Global
    New-Variable -Name "siufrules" -Value "HKCU:\Software\Microsoft\Siuf\Rules" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLFSVC" -Value "HKLM:\SYSTEM\CurrentControlSet\Services\lfsvc\Service\Configuration" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToWifiSense" -Value "HKLM:\Software\Microsoft\PolicyManager\default\WiFi" -Option ReadOnly -Scope Global
    New-Variable -Name "RegCAM" -Value "HKLM:\Software\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\location" -Option ReadOnly -Scope Global
    New-Variable -Name "Website" -Value "https://www.mothercomputers.com" -Option ReadOnly -Scope Global
    New-Variable -Name "Hours" -Value "Monday - Saturday 9AM-5PM | Sunday - Closed"  -Option ReadOnly -Scope Global
    New-Variable -Name "Phone" -Value "(250) 479-8561" -Option ReadOnly -Scope Global
    New-Variable -Name "Store" -Value "Mother Computers" -Option ReadOnly -Scope Global
    New-Variable -Name "Model" -Value "Mother Computers - (250) 479-8561" -Option ReadOnly -Scope Global
    New-Variable -Name "Page" -Value "Model" -Option ReadOnly -Scope Global

    New-Variable -Name "TimeoutScreenBattery" -Value 5 -Scope Global
    New-Variable -Name "TimeoutScreenPluggedIn" -Value 10 -Scope Global
    New-Variable -Name "TimeoutStandByBattery" -Value 15 -Scope Global
    New-Variable -Name "TimeoutStandByPluggedIn" -Value 30 -Scope Global
    New-Variable -Name "TimeoutDiskBattery" -Value 15 -Scope Global
    New-Variable -Name "TimeoutDiskPluggedIn" -Value 30 -Scope Global
    New-Variable -Name "TimeoutHibernateBattery" -Value 15 -Scope Global
    New-Variable -Name "TimeoutHibernatePluggedIn" -Value 30 -Scope Global
    New-Variable -Name "PathToUsersControlPanelDesktop" -Value "Registry::HKEY_USERS\.DEFAULT\Control Panel\Desktop" -Scope Global
    New-Variable -Name "PathToCUControlPanelDesktop" -Value "HKCU:\Control Panel\Desktop" -Scope Global
    New-Variable -Name "PathToCUGameBar" -Value "HKCU:\SOFTWARE\Microsoft\GameBar" -Scope Global
    New-Variable -Name "PathToOEMInfo" -Value "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\OEMInformation" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegExplorerLocalMachine" -Value "HKLM:\Software\Microsoft\Windows\CurrentVersion\Explorer" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegSystem" -Value "HKLM:\Software\Policies\Microsoft\Windows\System" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegInputPersonalization" -Value "HKCU:\Software\Microsoft\InputPersonalization" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegCurrentVersion" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegContentDelivery" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegExplorer" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegExplorerAdv" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegAdvertising" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion\AdvertisingInfo" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegPersonalize" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToPrivacy" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion\Privacy" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToRegSearch" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion\Search" -Option ReadOnly -Scope Global
    # Initialize all Path variables used to Registry Tweaks
    New-Variable -Name "PathToLMActivityHistory" -Value "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLMAutoLogger" -Value "HKLM:\SYSTEM\CurrentControlSet\Control\WMI\AutoLogger" -Option ReadOnly -Scope Global
    #$PathToLMDeliveryOptimizationCfg = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\DeliveryOptimization\Config"
    New-Variable -Name "PathToLMPoliciesAdvertisingInfo" -Value "HKLM:\SOFTWARE\Policies\Microsoft\Windows\AdvertisingInfo" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLMPoliciesCloudContent" -Value "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLMPoliciesEdge" -Value "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Scope Global
    New-Variable -Name "PathToLMPoliciesMRT" -Value "HKLM:\SOFTWARE\Policies\Microsoft\MRT" -Scope Global
    New-Variable -Name "PathToLMPoliciesPsched" -Value "HKLM:\SOFTWARE\Policies\Microsoft\Psched" -Scope Global
    New-Variable -Name "PathToLMPoliciesSQMClient" -Value "HKLM:\SOFTWARE\Policies\Microsoft\SQMClient\Windows" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLMPoliciesTelemetry" -Value "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLMPoliciesTelemetry2" -Value "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\DataCollection" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLMPoliciesToWifi" -Value "HKLM:\Software\Microsoft\PolicyManager\default\WiFi" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLMPoliciesWindowsStore" -Value "HKLM:\SOFTWARE\Policies\Microsoft\WindowsStore" -Scope Global
    #$PathToLMPoliciesWindowsUpdate = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU"
    New-Variable -Name "PathToLMWindowsTroubleshoot" -Value "HKLM:\SOFTWARE\Microsoft\WindowsMitigation" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUAccessibility" -Value "HKCU:\Control Pane\Accessibility" -Scope Global
    New-Variable -Name "PathToCUContentDeliveryManager" -Value "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUDeviceAccessGlobal" -Value "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\DeviceAccess\Global" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUExplorer" -Value "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer" -Scope Global
    New-Variable -Name "PathToCUExplorerAdvanced" -Value "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Scope Global
    New-Variable -Name "PathToCUInputPersonalization" -Value "HKCU:\SOFTWARE\Microsoft\InputPersonalization" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUInputTIPC" -Value "HKCU:\SOFTWARE\Microsoft\Input\TIPC" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUOnlineSpeech" -Value "HKCU:\SOFTWARE\Microsoft\Speech_OneCore\Settings\OnlineSpeechPrivacy" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUPoliciesCloudContent" -Value "HKCU:\SOFTWARE\Policies\Microsoft\Windows\CloudContent" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUSearch" -Value "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUSiufRules" -Value "HKCU:\SOFTWARE\Microsoft\Siuf\Rules" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToCUUserProfileEngagemment" -Value "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\UserProfileEngagement" -Scope Global
    New-Variable -Name "PathToVoiceActivation" -Value "HKCU:\Software\Microsoft\Speech_OneCore\Settings\VoiceActivation\UserPreferenceForAllApps" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToBackgroundAppAccess" -Value "HKCU:\Software\Microsoft\Windows\CurrentVersion\BackgroundAccessApplications" -Option ReadOnly -Scope Global
    New-Variable -Name "PathToLMMultimediaSystemProfile" -Value "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" -Scope Global
    New-Variable -Name "PathToLMMultimediaSystemProfileOnGameTasks" -Value "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile\Tasks\Games" -Scope Global
    ...
    New-Variable -Name "EnableScheduledTasks" -Value @(
    ...
    New-Variable -Name "DisableScheduledTasks" -Value @(
    ...
    #New-Variable -Name "IsSystemDriveSSD" -Value $(Get-OSDriveType) -eq "SSD" -Scope Global

    # - Services
    New-Variable -Name "EnableServicesOnSSD" -Value @("SysMain", "WSearch", "fhsvc") -Scope Global


    # Services which will be totally disabled
    New-Variable -Name "ServicesToDisabled" -Value @(
    ...
    New-Variable -Name "ServicesToManual" -Value @(
    ...
    # - Content Delivery
    New-Variable -Name "ContentDeliveryManagerDisableOnZero" -Value @(
    ...
    # - Optional Features
    New-Variable -Name "DisableFeatures" -Value @(
    ...
    New-Variable -Name "EnableFeatures" -Value @(
...
Function Optimize-Privacy {
    ...
    Set-ItemPropertyVerified -Path $PathToCUUserProfileEngagemment -Name "ScoobeSystemSettingEnabled" -Value $Zero -Type DWord
    ...
    If ($Revert) {
        ...
        Remove-ItemProperty -Path $PathToLMPoliciesTelemetry2 -Name "AllowTelemetry" -Force -ErrorAction SilentlyContinue
        ...
        Remove-ItemProperty -Path $PathToRegInputPersonalization -Name "RestrictImplicitTextCollection" -Force -ErrorAction SilentlyContinue
        Remove-ItemProperty -Path $PathToRegInputPersonalization -Name "RestrictImplicitInkCollection" -Force -ErrorAction SilentlyContinue
...
Function Optimize-Security {
    ...
    Set-ItemPropertyVerified -Path "$PathToLMPoliciesEdge\PhishingFilter" -Name "EnabledV9" -Type DWord -Value $One

    Write-Status -Types $EnableStatus[1].Symbol, $TweakType -Status "$($EnableStatus[1].Status) 'SmartScreen' for Store Apps..."
    Set-ItemPropertyVerified -Path "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppHost" -Name "EnableWebContentEvaluation" -Type DWord -Value $One
    ...
    Set-ItemPropertyVerified -Path "HKLM:\SOFTWARE\Microsoft\.NETFramework\v4.0.30319" -Name "SchUseStrongCrypto" -Type DWord -Value $One
    Set-ItemPropertyVerified -Path "HKLM:\SOFTWARE\Wow6432Node\Microsoft\.NETFramework\v4.0.30319" -Name "SchUseStrongCrypto" -Type DWord -Value $One

    Write-Section "Autoplay and Autorun (Removable Devices)"
    Write-Status -Types "-", $TweakType -Status "Disabling Autoplay..."
    Set-ItemPropertyVerified -Path "$PathToCUExplorer\AutoplayHandlers" -Name "DisableAutoplay" -Type DWord -Value $One
    ...
    Set-ItemPropertyVerified -Path "$PathToCUExplorerAdvanced" -Name "HideFileExt" -Type DWord -Value $Zero
    ...
    Set-ItemPropertyVerified -Path "$PathToLMPoliciesMRT" -Name "DontOfferThroughWUAU" -Type DWord -Value $Zero
}
...
Function Remove-Office {
    ...
    switch ($msgBoxInput) {
        'Yes' {
            ...
            $SaRAcmdexe = (Get-ChildItem "$Sexp" -Include SaRAcmd.exe -Recurse).FullName
        ...
        if ($appxPackageToRemove) {
            ...
            if ($PSCmdlet.ShouldProcess($actionDescription, "Remove-AppxPackage")) {
                $appxPackageToRemove | ForEach-Object -Process {
                    ...
                    If ($?) { Get-Status ; $Global:Removed++ ; $Global:PackagesRemoved += $appxPackageToRemove.PackageFullName } elseif (!($?)) { Get-Status ; $Global:Failed++ }
                }
                ...
                if ($PSCmdlet.ShouldProcess($actionDescription, "Remove-AppxProvisionedPackage")) {
                    ...
                    If ($?) { $Global:Removed++ ; $Global:PackagesRemoved += "Provisioned Appx $($appxPackageToRemove.PackageFullName)" } elseif (!($?)) { $Global:Failed++ }
                }
            }
        }
        else {
...
Function Set-Branding {
    ...
    elseif ($Revert) {
        ...
        if ($PSCmdlet.ShouldProcess($actionDescription)) {
            ...
            Set-ItemPropertyVerified -Path "$PathToOEMInfo" -Name "Manufacturer" -Type String -Value ""
            Write-Status -Types "-", $TweakType -Status "Removing Mothers Number from Support Page"
            Set-ItemPropertyVerified -Path "$PathToOEMInfo" -Name "SupportPhone" -Type String -Value ""
            Write-Status -Types "-", $TweakType -Status "Removing Store Hours from Support Page"
            Set-ItemPropertyVerified -Path "$PathToOEMInfo" -Name "SupportHours" -Type String -Value ""
            Write-Status -Types "-", $TweakType -Status "Removing Store URL from Support Page"
            Set-ItemPropertyVerified -Path "$PathToOEMInfo" -Name "SupportURL" -Type String -Value ""
            Write-Status -Types "-", $TweakType -Status "Removing Store Number from Settings Page"
            Set-ItemPropertyVerified -Path "$PathToOEMInfo" -Name "$page" -Type String -Value ""
        }
    }
}
...
function Set-Wallpaper {
    ...
    if ($PSCmdlet.ShouldProcess("Apply Wallpaper")) {
        $WallpaperPathExists = Test-Path $wallpaperPath -ErrorAction SilentlyContinue
        If (!$WallpaperPathExists) {
            ...
            Start-BitsTransfer -Source $WallpaperURL -Destination $WallpaperDestination -Dynamic
            Get-Status
        }
...
Function Send-EmailLog {
    ...
    $WallpaperApplied = if ($CurrentWallpaper -eq $Wallpaper) { "YES" } else { "NO" }
...
Function Show-ScriptStatus() {
    ...
    If ($TitleCounterText) {
        Write-TitleCounter -Counter $Counter -MaxLength $MaxLength -Text $TitleCounterText
    }
    If ($TitleText) {
    ...
    If (!$Revert) {
        ...
        ForEach ($Program in $Programs) {
            ...
            if ($PSCmdlet.ShouldProcess($target, "Remove UWP App")) {
                ...
                $CurrentItem++
                $PackagesRemoved = "$PackagesRemoved`n  - $Program"
...
.DESCRIPTION
    The Write-StyledCaption function is used to display a caption in the console with a visually distinct style. The caption is created using characters such as "==" and ">", and it is highlighted with customizable foreground and background colors based on the caption type.

.PARAMETER Text
    Specifies the text to be displayed as the caption. The default value is "No Text" if no value is provided.

.PARAMETER Type
    Specifies the type of the caption (failed, success, warning, none - default).

.EXAMPLE
    Write-StyledCaption -Text "Important Note" -Type "success"

    DESCRIPTION
        Displays a success caption "==> Important Note" with a distinct visual style in the console.

.EXAMPLE
    Write-StyledCaption -Text "Task execution failed!" -Type "failed"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUCuohORxE6l0A4JvQtXyqZtVZ
# NLOgggMkMIIDIDCCAgigAwIBAgIQU3x04/OYsb1NqZt3cgzIXTANBgkqhkiG9w0B
...
# gjcCARUwIwYJKoZIhvcNAQkEMRYEFLtMqo5H+gfiaDAmV/AO1QEZY6oeMA0GCSqG
# SIb3DQEBAQUABIIBAKi9yOLexuq1jJ1hTB9adIO0eChu/0+/1CxouKZRTXcgUTLc
# 2fbJ53VAeSEWbCPeUjWIfPvbUIJfBi2sOyk8ENOww65G/g4JCgya2HAV/g+Y0edL
# 4MXPl7nzudhSdbDXgIPZAd6c67UBrG8h73zP5YDC4z4tG9TgCqmij1avGAn6lgvc
# r5udFuvtim9SqFeEaF9xJJtXWvy7jM0+UIHui4TgHIb/+mKMlNhHgWovrlDClYQz
# RSbRcUntTxbT+1qLopxi2lCbIqb4iY3CkQ2ZEdeZF0wAAn8PJbXlA1QWyb4jNKGp
# e0WseErXVwMUNDyDQYBJImA3aLFh1dpFG6/vHoM=
